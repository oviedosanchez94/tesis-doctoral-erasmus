...
export default function Sidebar({ active, onNavigate, mobileOpen = false, onMobileClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 h-full bg-white border-r border-gray-100 flex-col transition-all duration-300 z-30
          ${collapsed ? 'w-16' : 'w-60'}
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-5 border-b border-gray-100">
          ...
          <button
            ...
            className="w-7 h-7 flex items-center justify-center rounded-lg text-gray-400 hover:bg-gray-50 hover:text-gray-600 transition-colors ml-auto hidden md:flex"
          >
            {collapsed ? '→' : '←'}
          </button>
          {/* Mobile close button */}
          <button
            onClick={onMobileClose}
            className="w-7 h-7 flex items-center justify-center rounded-lg text-gray-400 hover:bg-gray-50 hover:text-gray-600 transition-colors ml-auto md:hidden"
          >
            ✕
          </button>
        </div>
